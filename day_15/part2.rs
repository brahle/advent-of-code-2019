...
fn main() {
    ...
    queue.push_back(*answer.borrow());
    dist.insert(*answer.borrow(), 0);
    let mut max_distance = RefCell::new(0);
    ...
    while !queue.is_empty() {
        ...
        for delta in deltas.borrow().iter() {
            ...
            if (new_distance > *max_distance.borrow()) {
                *max_distance.borrow_mut() = new_distance;
            }
    ...
    println!("Max distance is {}", max_distance.borrow());
